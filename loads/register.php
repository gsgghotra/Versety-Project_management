...
<!-- JavaScript functions for form validation and registration -->
<script type="text/javascript">
function reg_auth(){
	if ($('#reg_u_email').val().length == 0 || $('#reg_u_user').val().length == 0 || $('#reg_u_pass').val().length == 0 || $('#reg_u_repass').val().length == 0) {
		reg_u_empty();
	} else {
		$.post('calendar/ajax/reg_auth.php',
			{
				regname: $('#reg_u_user').val(),
				regemail: $('#reg_u_email').val(),
				regpass: $('#reg_u_pass').val(),
				regrepass: $('#reg_u_repass').val()
			},
			function(data){
				if (data == 'success'){
					reg_u_pass();
...
// Function for handling empty fields
...
function reg_u_pass() {
	...
	// Hide form fields and registration button
	$('#reg_u_user').hide();
	$('#reg_u_pass').hide();
	$('#reg_u_email').hide();
	$('#reg_u_repass').hide();
...
// Function for username already taken
function reg_u_useral() {
	$(".alert").show();
	$(".reg_authaction").show();
	$('.reg_authaction').text('Username already exists. Please choose another username.');
	$('#reg_u_user').css("border-right", "1px solid red");
	$('#reg_u_pass').css("border-right", "0px solid ");
	$('#reg_u_email').css("border-right", "0px solid ");
	$('#reg_u_repass').css("border-right", "0px solid ");
...
function reg_u_invalidemail() {
	$(".alert").show();
	$(".reg_authaction").show();
	$('.reg_authaction').text('Please enter a correct email address.');
	$('#reg_u_user').css("border-right", "0px solid");
	$('#reg_u_pass').css("border-right", "0px solid ");
	$('#reg_u_email').css("border-right", "1px solid red");
	$('#reg_u_repass').css("border-right", "0px solid ");
...
// Functions for password errors
function reg_u_passsim() {
	$(".alert").show();
	$(".reg_authaction").show();
	$('.reg_authaction').text('Passwords do not match.');
...
function reg_u_notpass() {
	$(".alert").show();
	$(".reg_authaction").show();
	$('.reg_authaction').text('Password must contain both letters and numbers.');
	$('#reg_u_user').css("border-right", "0px solid red");
	$('#reg_u_pass').css("border-right", "1px solid ");
	$('#reg_u_email').css("border-right", "0px solid ");
	$('#reg_u_repass').css("border-right", "0px solid ");
